...
- [X] T054 [US6] `self-healer stop` + pid file — `start` writes `data/self-healer.pid` (detached child); new `stop` reads it, SIGTERMs, waits for graceful exit, clears the file; stale-pid handling; `status` shows `daemon: running (pid N) | stopped`. Pure helpers in `src/cli/pidfile.ts`; unit tests incl. a real SIGTERM stop against a spawned child. Smoke also surfaced two pre-existing daemon-start bugs, found because the detached child path was previously unreachable: (a) `src/paths.ts` fallback resolved one level too high (`"../.."` assumed depth-2, but paths.ts sits at src/) → `start` spawned a non-existent `…/Desktop/dist/daemon.mjs` → fixed to `".."` (+ `tests/unit/paths_test.ts`); (b) T053's argv guard also matched `dist/daemon.mjs`, so the bundle ran `startDaemon()` twice (daemonEntry + guard) → double webhook bind → EADDRINUSE → Fatal → dead daemon on every `start` → guard now matches dev `src/index.ts` only (+ `tests/unit/daemon_boot_test.ts` spawning the real bundle)
- [X] T055 Notification-spam docs — README "One-time setup: mute `ci` workflow notifications" (Settings → Actions → Notifications; workflow still reports to the agent) + `enable` success-message hint, so consumers don't hit perpetual failure emails on flaky reruns
- [X] T056 [US6] `enable` write-path live fix
- [X] T057 Classifier infra false-positive fix — live onboarding run surfaced bare `429` matching inside a GitHub Actions temp-dir UUID (`_temp/27bc7100-1591-4291-bd6e…`) → real lint failure (`File content differs from formatting output`) misclassified as `infra` and escalated instead of fixed. `rate_limit`/`expired_credentials`/`service_unavailable` patterns matched bare `401`/`429`/`503` anywhere; now they require HTTP-ish context (`HTTP 429`, `status: 429`, `429 Too Many Requests`, `401 unauthorized`, …). Regression tests: UUID-suffixed logs → no infra; bare `401`/`429`/`503` in innocuous text → no infra; keyword forms still flag. — live test on a fresh repo surfaced a v1.0.1 bug: `gh pr create` does not accept `--jq .html_url` (that flag is `gh pr view`/`gh api` territory), so `enable` on a repo without the reporter died after the branch + file commit and never opened the PR / never recorded `watched_repos`. Fix: build the `gh pr create` argv in a pure exported `prCreateArgs()` (no `--jq`; the PR URL comes from stdout, which `gh()` already trims) + regression test asserting no `--jq`; re-verified live on a fresh repo → reporter PR opened + `watched_repos` row recorded
